...
from linear_regression import LinearRegression, LinearRegressionLeastSquares, \
    ...
    LinearRegressionRidge,LinearRegressionOls, LinearRegressionMp, LinearRegressionOmp
...
class TestLinearRegressionOls(unittest.TestCase):
    def test_fit_identity(self):
        # Choose dimensions
        n_samples = 20
        n_features = n_samples
        ...
        # Estimate solution
        ols_reg = LinearRegressionOls(n_iter=n_iter)
        ols_reg.fit(X=X, y=y)
        w_est = ols_reg.w
        b_est = ols_reg.b
